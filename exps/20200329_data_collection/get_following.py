...
def get_following(api, screen_name, outpath):
    ...
    for page in tweepy.Cursor(api.friends_ids, screen_name=screen_name).pages():
        ...
        print('{} followings collected'.format(int(len(following_id_list))))
    if not os.path.exists(outpath):
        os.makedirs(outpath)
    with open(outpath + '{}_following.txt'.format(screen_name), 'w') as g:
        g.write(str(following_id_list))
...
def main(args):
    ...
    for member in members:
        get_following(api, member, args.outpath)
